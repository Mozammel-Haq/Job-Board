...
const loadChartData = async (period: 'Week' | 'Month' | 'Year') => {
  try {
    ...
    if (period === 'Week') {
      ...
      const response = await api.getYearlyStats();
      data = response.data || [];
            ...
            <div className="flex gap-2 bg-background-secondary rounded-lg p-1">
              {['Week', 'Month', 'Year'].map((period) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium transition-all rounded-md ${selectedPeriod === period
          ...
          <div className="flex gap-6 mb-6 border-b border-gray-100">
            {['Overview', 'Jobs View', 'Jobs Applied'].map((tab) => (
              <button
                ...
                className={`pb-3 font-medium transition-colors relative ${selectedTab === tab ? 'text-primary' : 'text-body hover:text-primary'
                  }`}
              >
                {tab}
                {selectedTab === tab && (
